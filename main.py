...
def typing_errors(time_passed:list, example_text:str):
    print(time_passed, example_text)
...
def typing_test():
    time_passed = []
    example_text = read_random_text_part(config["character_count"],
                                         convert_german_letters=config["convert_german_letters"])
...
def analyze_typing_test(typing_test_data):
    ...
    print(f"typing speed:  ({len(df)} character_count)")
...
def print_event(event):
    print(end=f"\nEvent from {datetime.fromtimestamp(event['start_time'])}   ")
